...
class DataPerangkatKerasController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('tipe', function ($row) {
                return $row->tipe ? $row->tipe : '';
            });
            $table->editColumn('serial_number', function ($row) {
                return $row->serial_number ? $row->serial_number : '';
            });
            $table->editColumn('tahun_beli', function ($row) {
                return $row->tahun_beli ? $row->tahun_beli : '';
            });
            $table->editColumn('nomor_u', function ($row) {
                return $row->nomor_u ? $row->nomor_u : '';
            });

            $table->editColumn('kontak_pic', function ($row) {
                return $row->kontak_pic ? $row->kontak_pic : '';
            });
            $table->editColumn('telepon', function ($row) {
                return $row->telepon ? $row->telepon : '';
            });
            $table->editColumn('perusahaan', function ($row) {
                return $row->perusahaan ? $row->perusahaan : '';
            });
            $table->editColumn('pengampu', function ($row) {
                return $row->pengampu ? $row->pengampu : '';
            });
